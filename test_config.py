...
from config import ConfigurationError, PROJECT_DIR, Settings, get_settings
...
class ConfigurationTests(unittest.TestCase):
    ...
    def test_defaults_and_project_relative_paths(self):
        ...
        self.assertEqual(Settings(_env_file=None, chroma_path="other").chroma_path,
                         (PROJECT_DIR / "other").resolve())

    def test_environment_overrides_dotenv_and_defaults(self):
        with tempfile.TemporaryDirectory() as directory:
            dotenv = Path(directory) / ".env"
            dotenv.write_text('OLLAMA_MODEL=from-file\nCORS_ALLOWED_ORIGINS=["http://localhost:3000"]\n')
    ...
    def test_rejects_invalid_values(self):
        for values in [
            {"ollama_model": " "}, {"chroma_collection": " "}, {"chroma_path": ""},
            {"ollama_timeout_seconds": 0}, {"ollama_timeout_seconds": float("inf")},
            {"healthcheck_timeout_seconds": -1}, {"healthcheck_timeout_seconds": float("nan")},
            {"ollama_base_url": "file:///tmp"}, {"ollama_base_url": "http://user:secret@localhost"},
    ...
    def test_startup_error_does_not_echo_bad_configuration(self):
        with patch.dict(os.environ, {"OLLAMA_BASE_URL": "http://private-user:private-secret@localhost"}):
    ...
    def test_settings_are_cached_and_immutable(self):
        with patch("config.Settings", return_value=Settings(_env_file=None)) as factory:
